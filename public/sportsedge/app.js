...
function renderWeek(w){ if(!w||!w.games||!w.games.length){ weekEl.innerHTML='<p class="small">No picks yet. Update /public/data/nfl/current.json</p>'; return;}
...
(async function init(){
  ...
  const current = await loadJSON('/data/nfl/current.json', null);
  const history = await loadJSON('/data/nfl/history.json', {weeks:[]});
